...
var _loop = function _loop(_i) {
  ullist[_i].onmouseover = function () {
    for (var j = 0; j < ullist.length; j++) {
      ullist[j].className = '';
      ollist[j].className = '';
    }

    ullist[_i].className = 'li1';
    ollist[_i].className = 'show1';
  };
};

for (var _i = 0; _i < ullist.length; _i++) {
  _loop(_i);
}
...
window.onscroll = function () {
  //获取当前的滚动距离
  top1 = document.documentElement.scrollTop; //判断多少显示
...
retrun1.onclick = function () {
  ...
  var dsq = setInterval(function () {
    //，每0.2秒钟走总滚动距离的10分之一
    var span = Math.ceil(top1 / 10); //获取剩下需要走的总滚动距离

    document.documentElement.scrollTop = top1 - span; //如果滚动距离小于0时，关闭定时器
